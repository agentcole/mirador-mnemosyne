...
const COLLECTION_API = document.getElementById("mirador").dataset.miradorCollectionApi;
const MANIFEST_PREFIX = document.getElementById("mirador").dataset.miradorManifestPrefix;
...
export const updateViewers = (workspace, manifestKeys, miradorInstance) => {
  manifestKeys.forEach((id) => {
    ...
    console.log("viewer", id, viewerWindow);
    if (viewerWindow) {
      setTimeout(() => {
        miradorInstance.store.dispatch({
